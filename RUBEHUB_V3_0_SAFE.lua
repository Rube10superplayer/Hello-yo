-- RubeHub V3.0 SAFE | Credits: RubeTheCoder
-- ═══════════════════════════════════════════════════════════════
-- WHAT WAS BREAKING AND WHY — FULL ROOT CAUSE ANALYSIS
-- ═══════════════════════════════════════════════════════════════
--
-- BUG 1: "Humanoid is not a valid member of Model Ship_Steward"
--   inside ReplicatedStorage.Effect.Container.Death
--   ROOT CAUSE: Code accessed model.Humanoid directly on NPC/Ship models.
--   Ships like Ship_Steward, Sea Beast, event NPCs have NO Humanoid object.
--   Direct indexing throws an error rather than returning nil.
--   FIX: Every NPC Humanoid access replaced with FindFirstChildOfClass("Humanoid")
--        which safely returns nil instead of throwing. All 118 occurrences fixed.
--
-- BUG 2: "attempt to call a nil value" LocalScript Line 1
--   ROOT CAUSE A: getrawmetatable/setreadonly/newcclosure/getnamecallmethod
--     were called completely UNGUARDED inside a bare spawn(). If ANY of those
--     4 functions is nil in the executor, spawn() creates a new coroutine and
--     calling nil inside it is reported as "LocalScript Line 1".
--   ROOT CAUSE B: getgc() + hookfunction() loops (V2.8 METHOD 2 and METHOD 3)
--     tried to hook game-internal function closures. hookfunction on certain
--     BF 2026 closures crashes with nil-call inside the hook wrapper itself.
--   ROOT CAUSE C: Effect.Container Death/Respawn modules were being
--     hookfunction'd after require(). After the 2026 update these module
--     functions are protected closures — hookfunction on them throws.
--   FIX A: spawn() metatable block fully pcall-wrapped with upfront nil checks.
--   FIX B: getgc hookfunction METHOD 2 and METHOD 3 removed entirely.
--   FIX C: Effect.Container only uses pcall(require) — no hookfunction at all.
--
-- ✅ V3.0: All 118 unsafe .Humanoid accesses fixed with FindFirstChildOfClass
-- ✅ V3.0: Effect.Container Death/Respawn hookfunction removed (pcall-require only)
-- ✅ V3.0: getgc hookfunction METHOD 2 and 3 fully removed
-- ✅ V3.0: spawn() metatable hook fully pcall-guarded with nil checks
-- ✅ V3.0: Fast Attack kept, uses CombatFramework safely (no getgc)
-- ✅ V3.0: All exploit function no-op fallbacks added at startup
-- ✅ V3.0: Valentine features removed (carried from V2.9)
-- ═══════════════════════════════════════════════════════════════
...
getfenv            = getfenv            or debug and debug.getfenv or function() return {} end
                    ...
                    if getfenv(v1175).script == game.Players.LocalPlayer.Character:WaitForChild("Soru") then
                        for v1176, v1177 in pairs(debug.getupvalues(v1175)) do
                            if type(v1177) == "table" and v1177.LastUse then
                                ...
                                until not InfiniteSoru or (function() local _c=game:GetService("Players").LocalPlayer.Character local _h=_c and _c:FindFirstChildOfClass("Humanoid") return not _h or _h.Health<=0 end)()
                            end
                        end
                    end
                end
                    ...
                    if getfenv(v1185).script == game.Players.LocalPlayer.Character:WaitForChild("Geppo") then
                        for v1186, v1187 in next, getupvalues(v1185) do
                            if tostring(v1187) == "0" then
                                ...
                                until not InfiniteGeppo or (function() local _c=game:GetService("Players").LocalPlayer.Character local _h=_c and _c:FindFirstChildOfClass("Humanoid") return not _h or _h.Health<=0 end)()
                            end
                        end
                    end
                end
