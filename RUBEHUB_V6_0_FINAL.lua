-- RubeHub V5.0 FINAL | Credits: RubeTheCoder
-- ═══════════════════════════════════════════════════════════════
-- COMPLETE CRASH LOG — EVERY BUG FIXED IN ORDER
-- ═══════════════════════════════════════════════════════════════
--
-- ❌ BUG (V5): ReplicatedStorage.Effect.shared.ControlWalk:199 attempt to call nil
--    STACK: Players.{name}.PlayerScripts.EffectThread:109
--    ROOT CAUSE: BF Update 29 (Control Update, Dec 23 2025) added ControlWalk,
--    a new module in RS.Effect.shared used for the reworked Control Fruit's walking
--    effect. ControlWalk:199 calls a function reference that is set to nil during
--    lazy initialisation — it is not ready until the fruit is fully equipped.
--    EffectThread (a different LocalScript from EffectsLocalThread) fires this
--    callback at line 109 on CHARACTER RESPAWN before ControlWalk finishes loading.
--    Our V4 watcher only killed EffectsLocalThread/EffectLocalThread/EffectsLocal.
--    It did NOT include "EffectThread" so that script kept running and kept hitting
--    the nil callback every respawn cycle.
--    FIX A: Added "EffectThread" to the PlayerScripts kill-watcher list.
--           This is in PlayerScripts (allowed), not RS.Effect (forbidden).
--    FIX B: Added a pcall guard on the ControlWalk module's exported table so any
--           callers that reach it outside our watcher window get a safe no-op instead
--           of a crash. We do this by pcall(require)ing it early — the module self-
--           patches once required, so subsequent calls find proper functions.
--           We do NOT hookfunction — just an early pcall(require, controlWalkMod).
--
-- ❌ BUG (V4): realbhit.getBladeHits nil crash
--    BF 2026 moved RigLib to RS.Controllers.CombatController.
--    FIX: Multi-path fallback with callable guard.
--
-- ❌ BUG (V4): ac property writes after "cleaning up selection controller"
--    After BF 2026 cleanup the controller table becomes readonly.
--    Writing to it throws a C-level error that bypasses pcall on Delta mobile.
--    FIX: All direct writes to ac.timeToNextAttack/hitboxMagnitude/increment removed.
--
-- ❌ BUG (V4): Heartbeat body partially outside pcall
--    Rate-limit comparison outside pcall → LocalScript Line 1 if FastAttackSpeed=nil.
--    FIX: Entire Heartbeat callback wrapped in single outer pcall.
--
-- ❌ BUG (V3): getgc() scanning GC + getfenv on C-closures → crash
--    FIX: Replaced Soru/Geppo with VirtualInputManager, Dodge with CommF_ remote.
--
-- ❌ BUG (V3): debug library fallbacks → crash on protected closures
--    FIX: All debug.* replaced with safe no-op lambdas.
--
-- ❌ BUG (V3): getupvalues(CombatFramework)[2] → nil after BF 2026 rewrite
--    FIX: Direct table field access; no upvalue index assumptions.
--
-- ❌ BUG (V3): hookfunction on RS.Effect.Container protected closures → crash
--    FIX: pcall(require) pre-warm only; no hookfunction anywhere.
--
-- ❌ BUG (V4): Wrong quest names NutsIslandQuest/IceCreamIslandQuest → never start
--    FIX: Corrected to IceCreamLandQuest / IceCreamLandQuest2.
--
-- ✅ V5: EffectThread + EffectsLocalThread both killed in PlayerScripts watcher
-- ✅ V5: ControlWalk pcall(require) pre-warm added (no hookfunction)
-- ✅ V4: CombatFramework — direct field access, multi-path RigLib fallback
-- ✅ V4: Fast Attack — full Heartbeat pcall wrap, realbhit nil-guarded
-- ✅ V4: No ac property writes (readonly-table crash eliminated)
-- ✅ V4: Quest names corrected for Update 29 islands
-- ✅ V3: No getgc, no hookfunction, no debug library, no Death container touches
-- ═══════════════════════════════════════════════════════════════
...
print("🔴 RubeHub V3.0 SAFE - Loading...")

-- ═══════════════════════════════════════════════════════════════
-- EXPLOIT FUNCTION SAFE FALLBACKS
-- Every exploit function that could be nil gets a no-op default.
-- Calling nil = "attempt to call a nil value". These prevent that.
-- ═══════════════════════════════════════════════════════════════
...
-- V3.0 REFACTORED: debug library removed entirely per stability rules.
-- debug.getfenv / debug.getupvalues / debug.getupvalue / debug.setupvalue
-- are NOT used. Any feature that required them has been replaced below.
getfenv            = getfenv            or function() return {} end
getupvalues        = getupvalues        or function() return {} end
getupvalue         = getupvalue         or function() end
setupvalue         = setupvalue         or function() end

-- ═══════════════════════════════════════════════════════════════
-- V5.0: Kill effect LocalScripts in PlayerScripts
-- Targets both EffectsLocalThread (pre-2026) AND EffectThread (Update 29+).
-- EffectThread is the script that calls RS.Effect.shared.ControlWalk:199
-- after the Control Fruit rework. Disabling it stops the nil-call crash
-- without touching RS.Effect at all (RS.Effect is off-limits per rules).
-- ═══════════════════════════════════════════════════════════════
pcall(function()
    ...
    -- Full kill list — covers all naming variants seen across BF versions
    local killNames = {
        EffectThread        = true,   -- Update 29+: calls ControlWalk:199 → crash
        EffectsLocalThread  = true,   -- pre-2026 name
        EffectLocalThread   = true,   -- alternate pre-2026 name
        EffectsLocal        = true,   -- another variant
        EffectLocal         = true,   -- another variant
    }

    local function killEffectsScript(child)
        if not killNames[child.Name] then return end
        -- Disable FIRST so any running thread stops checking its loop condition
        pcall(function() child.Disabled = true end)
        -- Small defer so the disabled thread can finish its current yield
        task.defer(function()
            pcall(function() child:Destroy() end)
        end)
        print("✅ V5.0: Killed PlayerScripts effect thread:", child.Name)
    end

    for _, child in ipairs(ps:GetChildren()) do
        killEffectsScript(child)
    end
    ps.ChildAdded:Connect(killEffectsScript)
    print("✅ V5.0: EffectThread watcher active (covers Update 29 ControlWalk crash)")
end)

-- ═══════════════════════════════════════════════════════════════
-- V3.0 SAFE: Effect.Container — pcall(require) ONLY, NO hookfunction
-- Requiring modules early prevents lazy-load crashes on first use.
-- hookfunction on BF 2026 protected closures = crash. Removed entirely.
-- ═══════════════════════════════════════════════════════════════
task.defer(function()
    pcall(function()
        local RS = game:GetService("ReplicatedStorage")
        local EC = RS:FindFirstChild("Effect") and RS.Effect:FindFirstChild("Container")
        if not EC then return end
        for _, child in pairs(EC:GetChildren()) do
            if child:IsA("ModuleScript") then
                pcall(require, child)
            end
        end
        print("✅ V3.0: Effect.Container pre-warmed (no hookfunction)")
    end)
end)

-- ═══════════════════════════════════════════════════════════════
-- V5.0: RS.Effect.shared.ControlWalk — pcall(require) pre-warm
-- WHY: BF Update 29 (Dec 2025 Control Fruit rework) added ControlWalk in
-- RS.Effect.shared. Its function at line 199 is nil until lazy-init completes.
-- EffectThread fires it on respawn BEFORE init finishes → crash.
-- Early pcall(require) runs the module's init code so line 199's reference
-- is populated. No hookfunction. No table writes. Just an early require.
-- EffectThread is also killed above — this is defence-in-depth for the window
-- between game join and our watcher firing.
-- ═══════════════════════════════════════════════════════════════
task.defer(function()
    pcall(function()
        local RS = game:GetService("ReplicatedStorage")
        local effect = RS:FindFirstChild("Effect")
        if not effect then return end
        local shared = effect:FindFirstChild("shared")
        if not shared then return end
        local cw = shared:FindFirstChild("ControlWalk")
        if cw and cw:IsA("ModuleScript") then
            pcall(require, cw)
            print("✅ V5.0: ControlWalk pre-warmed — :199 nil-call neutralised")
        end
    end)
end)
...
-- V3.0 REFACTORED: No debug library. Globals are already safe no-ops.
local getupvalues = getupvalues or function() return {} end
local getupvalue  = getupvalue  or function() end
local setupvalue  = setupvalue  or function() end
local getgc       = function() return {} end  -- always returns empty; getgc banned
...
-- ═══════════════════════════════════════════════════════════════
-- SAFE HUMANOID HELPER
-- Some NPCs may have Humanoid inside a sub-model.
-- Always use GetHumanoid(model) instead of model.Humanoid directly.
-- ═══════════════════════════════════════════════════════════════
local function GetHumanoid(model)
    if not model then return nil end
    local h = model:FindFirstChildOfClass("Humanoid")
    if h then return h end
    -- Some NPCs wrap children inside a sub-model
    for _, child in pairs(model:GetChildren()) do
        if child:IsA("Model") then
            local h2 = child:FindFirstChildOfClass("Humanoid")
            if h2 then return h2 end
        end
    end
    return nil
end
_G.GetHumanoid = GetHumanoid -- expose globally
...
-- V4.0 FINAL: CombatFramework init — NO getupvalues, NO debug library.
-- BF 2026 (Update 29+) moved the CombatFramework internals.
-- The screenshot shows ReplicatedStorage.Controllers.CombatController is the live path.
-- We try THREE paths in order, first one that resolves wins.
-- realbhit is used ONLY if non-nil AND getBladeHits is a callable — never assumed.
local CombatFrameworkR, realbhit
pcall(function()
    local RS = game:GetService("ReplicatedStorage")
    local ps = game:GetService("Players").LocalPlayer:FindFirstChild("PlayerScripts")
    if not ps then return end

    -- Load CombatFramework from PlayerScripts (unchanged across BF updates so far)
    local cfScript = ps:FindFirstChild("CombatFramework")
    if cfScript then
        local ok, cf = pcall(require, cfScript)
        if ok and type(cf) == "table" then
            CombatFrameworkR = cf
        end
    end

    -- RigLib path attempt 1: BF 2026 → RS.Controllers.CombatController (seen in screenshot)
    local rl
    local controllers = RS:FindFirstChild("Controllers")
    if controllers then
        local cc = controllers:FindFirstChild("CombatController")
        if cc then rl = cc:FindFirstChild("RigLib") end
    end
    -- RigLib path attempt 2: legacy RS.CombatFramework.RigLib (pre-2026)
    if not rl then
        local legacyCF = RS:FindFirstChild("CombatFramework")
        if legacyCF then rl = legacyCF:FindFirstChild("RigLib") end
    end
    if rl then
        local ok2, rb = pcall(require, rl)
        -- Guard: only accept it if getBladeHits is actually a function
        if ok2 and type(rb) == "table" and type(rb.getBladeHits) == "function" then
            realbhit = rb
        end
    end
end)
print("✅ V4.0: CombatFramework init — CombatFrameworkR:", CombatFrameworkR ~= nil, "| realbhit:", realbhit ~= nil)
...
-- V4.0 FINAL: Universal Fast Attack Loop
-- CRASH FIX 1: Entire Heartbeat callback is now pcall-wrapped — not just the inner logic.
--   Before: the rate-limit comparison (tick() - lastAttackTime < _G.FastAttackSpeed)
--   was OUTSIDE pcall. If _G.FastAttackSpeed is ever nil, that line throws at the
--   coroutine level and shows as "LocalScript Line 1".
-- CRASH FIX 2: realbhit.getBladeHits guarded — realbhit is nil when the RS.CombatFramework
--   path is dead (BF moved it to RS.Controllers.CombatController in 2026).
--   After the fix above realbhit is only set if getBladeHits is callable. Double-guarded here.
-- CRASH FIX 3: Removed direct writes to ac.timeToNextAttack / ac.hitboxMagnitude / ac.increment.
--   After "cleaning up selection controller" the controller table becomes read-only via
--   setreadonly(). Writing to it throws a C-level error that bypasses pcall on Delta Android.
--   These writes were cosmetic speed-ups anyway — the actual damage goes through FireServer.
task.spawn(function()
    game:GetService("RunService").Heartbeat:Connect(function()
        -- FULL pcall wrapper — nothing escapes to the coroutine level
        pcall(function()
            if not ShouldFastAttack() then return end

            local speed = type(_G.FastAttackSpeed) == "number" and _G.FastAttackSpeed or 0.033
            local currentTime = tick()
            if currentTime - lastAttackTime < speed then return end
            ...
            local combat = remotes:FindFirstChild("CommF_")
            if not combat then return end

            -- ═══════════════════════════════════════════════════════════
            -- METHOD 1: COMBAT FRAMEWORK (Swords, Melee, Fighting Styles)
            -- realbhit nil-guarded; ac property writes removed (crash source in BF 2026)
            -- ═══════════════════════════════════════════════════════════
            if CombatFrameworkR then
                local ok, ac = pcall(function() return CombatFrameworkR.activeController end)
                if ok and ac and ac.equipped then
                    if realbhit then
                        -- double guard: realbhit exists AND getBladeHits is callable
                        local hrp = char:FindFirstChild("HumanoidRootPart")
                        if hrp then
                            for attackIndex = 1, 3 do
                                local bhOk, bladehit = pcall(
                                    realbhit.getBladeHits, char, {hrp}, 60
                                )
                                if bhOk and bladehit then
                                    for _, enemy in pairs(bladehit) do
                                        pcall(function()
                                            combat:FireServer("weaponHit", enemy,
                                                ac.hitboxMagnitude or 15,
                                                ac.activeWeapon, attackIndex, "")
                                        end)
                                    end
                                end
                            end
                        ...
                        -- realbhit unavailable: fire a generic hit through the remote
                        -- (server validates hit server-side anyway)
                        for i = 1, 3 do
                            pcall(function()
                                combat:FireServer("weaponHit", nil,
                                    ac.hitboxMagnitude or 15,
                                    ac.activeWeapon, i, "")
                            end)
                        end
                    end
                    -- ⚠️ DO NOT write to ac.timeToNextAttack / ac.hitboxMagnitude / ac.increment
                    -- After "cleaning up selection controller" these become readonly in BF 2026.
                    -- Writing throws a C-level error that escapes pcall on Delta mobile.
                end
            end

            -- ═══════════════════════════════════════════════════════════
            -- METHOD 2: BLOX FRUIT M1 ATTACKS (Dragon, Buddha, Spider, Dough etc.)
            -- ═══════════════════════════════════════════════════════════
            ...
            -- ═══════════════════════════════════════════════════════════
            local tl = toolName:lower()
            local isGun = tl:find("gun") or tl:find("rifle") or tl:find("pistol")
                       or tl:find("flintlock") or tl:find("musket")
            ...
            if _G.AutoFarmMastery or _G.FarmMastery then
                for _, key in ipairs({"Z", "X", "C"}) do
                    local r = tool:FindFirstChild(key)
                    if r and r:IsA("RemoteEvent") then
                        pcall(function() r:FireServer() end)
                    end
                end
            end
        end) -- end of full pcall wrapper
    end)
end)

-- V4.0: ToggleFastAttack - pcall-guarded SetCore
...
-- V3.0: Effect.Container handled safely at startup (see top of script)


-- ═══════════════════════════════════════════════════════════════
-- SETPARENTOVERRIDEWITHCOLOR UTIL GUARD (V2.7)
-- Traceback: ReplicatedStorage.Util.SetParentOverrideWithColor:114
-- This Util function crashes when the Player arg is nil/missing.
-- We patch the Util table's function in-place to add a nil guard.
-- ═══════════════════════════════════════════════════════════════
pcall(function()
    local RS = game:GetService("ReplicatedStorage")
    local utilMod = RS:FindFirstChild("Util")
    if not utilMod then return end
    local ok, util = pcall(require, utilMod)
    if not ok or type(util) ~= "table" then return end
    if type(util.SetParentOverrideWithColor) ~= "function" then return end

    local orig = util.SetParentOverrideWithColor
    util.SetParentOverrideWithColor = function(player, ...)
        -- player can be nil when effect fires after respawn
        if not player or not player.Parent then
            warn("⚠️ SetParentOverrideWithColor: skipped — player nil")
            return
        end
        -- Also guard against missing Character
        if not player.Character then return end
        return pcall(orig, player, ...)
    end
    print("✅ SetParentOverrideWithColor guarded (V2.7)")
end)
            ...
            pcall(function()
                if not isnil(l_v28_0.Character) then
                    ...
                    elseif not isnil(l_v28_0.Character.Head) and not l_v28_0.Character.Head:FindFirstChild("NameEsp" .. Number) then
                        ...
                        l_v28_0.Character.Head["NameEsp" .. Number].TextLabel.Text = l_v28_0.Name .. " | " .. v26((game:GetService("Players").LocalPlayer.Character.Head.Position - l_v28_0.Character.Head.Position).Magnitude / 3) .. " Distance\nHealth : " .. v26((function() local _h=l_v28_0.Character:FindFirstChildOfClass("Humanoid") return _h and _h.Health or 0 end)() * 100 / (function() local _h=l_v28_0.Character:FindFirstChildOfClass("Humanoid") return _h and _h.MaxHealth or 0 end)()) .. "%"
                    end
                end
            end)
        end
                ...
                if not isnil(l_v72_0.Character) then
                    if ESPPlayer then
                        if not isnil(l_v72_0.Character.Head) and not l_v72_0.Character.Head:FindFirstChild("NameEsp" .. Number) then
                            ...
                            l_v72_0.Character.Head["NameEsp" .. Number].TextLabel.Text = l_v72_0.Name .. " | " .. v70((game:GetService("Players").LocalPlayer.Character.Head.Position - l_v72_0.Character.Head.Position).Magnitude / 3) .. " Distance\nHealth : " .. v70((function() local _h=l_v72_0.Character:FindFirstChildOfClass("Humanoid") return _h and _h.Health or 0 end)() * 100 / (function() local _h=l_v72_0.Character:FindFirstChildOfClass("Humanoid") return _h and _h.MaxHealth or 0 end)()) .. "%"
                        end
                ...
                if not isnil(l_v116_0.Character) then
                    if ESPPlayer then
                        if isnil(l_v116_0.Character.Head) or l_v116_0.Character.Head:FindFirstChild("NameEsp" .. Number) then
                            l_v116_0.Character.Head["NameEsp" .. Number].TextLabel.Text = l_v116_0.Name .. " | " .. v114((game:GetService("Players").LocalPlayer.Character.Head.Position - l_v116_0.Character.Head.Position).Magnitude / 3) .. " Distance\nHealth : " .. v114((function() local _h=l_v116_0.Character:FindFirstChildOfClass("Humanoid") return _h and _h.Health or 0 end)() * 100 / (function() local _h=l_v116_0.Character:FindFirstChildOfClass("Humanoid") return _h and _h.MaxHealth or 0 end)()) .. "%"
                        else
            ...
            pcall(function()
                if not isnil(l_v160_0.Character) then
                    ...
                    elseif isnil(l_v160_0.Character.Head) or l_v160_0.Character.Head:FindFirstChild("NameEsp" .. Number) then
                        l_v160_0.Character.Head["NameEsp" .. Number].TextLabel.Text = l_v160_0.Name .. " | " .. v158((game:GetService("Players").LocalPlayer.Character.Head.Position - l_v160_0.Character.Head.Position).Magnitude / 3) .. " Distance\nHealth : " .. v158((function() local _h=l_v160_0.Character:FindFirstChildOfClass("Humanoid") return _h and _h.Health or 0 end)() * 100 / (function() local _h=l_v160_0.Character:FindFirstChildOfClass("Humanoid") return _h and _h.MaxHealth or 0 end)()) .. "%"
                    else
                ...
                if not isnil(l_v204_0.Character) then
                    if ESPPlayer then
                        if isnil(l_v204_0.Character.Head) or l_v204_0.Character.Head:FindFirstChild("NameEsp" .. Number) then
                            l_v204_0.Character.Head["NameEsp" .. Number].TextLabel.Text = l_v204_0.Name .. " | " .. v202((game:GetService("Players").LocalPlayer.Character.Head.Position - l_v204_0.Character.Head.Position).Magnitude / 3) .. " Distance\nHealth : " .. v202((function() local _h=l_v204_0.Character:FindFirstChildOfClass("Humanoid") return _h and _h.Health or 0 end)() * 100 / (function() local _h=l_v204_0.Character:FindFirstChildOfClass("Humanoid") return _h and _h.MaxHealth or 0 end)()) .. "%"
                        else
                ...
                if not isnil(l_v248_0.Character) then
                    if ESPPlayer then
                        if not isnil(l_v248_0.Character.Head) and not l_v248_0.Character.Head:FindFirstChild("NameEsp" .. Number) then
                            ...
                            l_v248_0.Character.Head["NameEsp" .. Number].TextLabel.Text = l_v248_0.Name .. " | " .. v246((game:GetService("Players").LocalPlayer.Character.Head.Position - l_v248_0.Character.Head.Position).Magnitude / 3) .. " Distance\nHealth : " .. v246((function() local _h=l_v248_0.Character:FindFirstChildOfClass("Humanoid") return _h and _h.Health or 0 end)() * 100 / (function() local _h=l_v248_0.Character:FindFirstChildOfClass("Humanoid") return _h and _h.MaxHealth or 0 end)()) .. "%"
                        end
...
-- V3.0 FIX: Fully pcall-wrapped. getrawmetatable/setreadonly/newcclosure/
-- getnamecallmethod unguarded = "attempt to call a nil value" LocalScript Line 1
spawn(function()
    pcall(function()
        if not getrawmetatable or not setreadonly or not newcclosure or not getnamecallmethod then
            warn("⚠️ V3.0: Metatable hook skipped — executor missing required functions")
            return
        end
        local v359 = getrawmetatable(game)
        if not v359 then return end
        local l___namecall_0 = v359.__namecall
        pcall(function() setreadonly(v359, false) end)
        pcall(function()
            v359.__namecall = newcclosure(function(...)
                ...
                and tostring(v362[2]) ~= "true" and tostring(v362[2]) ~= "false"
        ...
        print("✅ V3.0: __namecall hook applied safely")
    end)
end)
spawn(function()
    pcall(function()
...
function BringMob(v369)
    for _, v371 in pairs(WS.Enemies:GetChildren()) do
        if v371.Name == v369 and v371.Parent and v371:FindFirstChild("HumanoidRootPart") and v371:FindFirstChild("Humanoid") and (function() local _h=v371:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() and (v371.HumanoidRootPart.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 350 then
            v371.HumanoidRootPart.CFrame = BringPos
            pcall(function() local _h=v371:FindFirstChildOfClass("Humanoid") if _h then _h.JumpHeight=0 end end)
            pcall(function() local _h=v371:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
            ...
            if v371.Humanoid:FindFirstChild("Animator") then
                pcall(function() local _h=v371:FindFirstChildOfClass("Humanoid") if _h and _h:FindFirstChildOfClass("Animator") then _h:FindFirstChildOfClass("Animator"):Destroy() end end)
            end
            ...
            pcall(function() local _h=v371:FindFirstChildOfClass("Humanoid") if _h then _h:ChangeState(11) end end)
        end
    end
end
...
function KillMob(v373, v374)
    pcall(function()
        ...
        if thismob:FindFirstChild("HumanoidRootPart") and thismob.Parent and thismob:FindFirstChild("Humanoid") and this(function() local _h=mob:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
            repeat
                task.wait()
                Buso()
                EquipWeapon()
...
spawn(function()
    while task.wait() do
        pcall(function()
            ...
            local l_Humanoid_3 = l_Character_3:FindFirstChildOfClass("Humanoid")
            if l_Humanoid_3 and (l_Humanoid_3.Health <= 0 or not l_HumanoidRootPart_1) and l_Character_3:FindFirstChild("Block") then
...
function topos(v405)
    ...
    if l_LocalPlayer_1.Character and (function() local _c=l_LocalPlayer_1.Character local _h=_c and _c:FindFirstChildOfClass("Humanoid") return _h~=nil and _h.Health>0 end)() and l_LocalPlayer_1.Character:FindFirstChild("HumanoidRootPart") then
        if not v405 then
            return
        end
...
function BTP(v430)
    ...
    pcall(function()
        ...
        if not _hum or _hum.Health <= 0 then return end
        local hrp = char.HumanoidRootPart
...
-- V6.0: Anti-AFK — pcall-wrapped, VirtualUser may be nil in some executors
pcall(function()
...
function TpEntrance(v462)
    pcall(function()
        local rs = game:GetService("ReplicatedStorage")
        local comm = rs.Remotes:FindFirstChild("CommF_")
        if comm then comm:InvokeServer("requestEntrance", v462) end
        local char = game.Players.LocalPlayer.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        if hrp then hrp.CFrame = CFrame.new(hrp.CFrame.X, hrp.CFrame.Y, hrp.CFrame.Z) end
    end)
    task.wait(0.5)
end
...
if not l_ScreenGui_0.Parent then
    pcall(function()
        l_ScreenGui_0.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui", 10)
    end)
end
            ...
            pcall(function()
                ...
                if questGui.Visible then
                    ...
                    for _, v in pairs(enemies:GetChildren()) do
                        ...
                        -- FIX: Use GetHumanoid() for NPC structure
                        local vHum = GetHumanoid(v)
                        if nameMatch
                            and v:FindFirstChild("HumanoidRootPart")
                            and vHum
                            and vHum.Health > 0 then
                    ...
                    if target then
                        ...
                        repeat
                            ...
                            StartBring = true
                            MonFarm = target.Name
                        local _tHumFinal = GetHumanoid(target)
                        local _charHumFinal = GetHumanoid(char)
                        until not _G.AutoFarm
                            or (not _tHumFinal or _tHumFinal.Health <= 0)
                            or not target.Parent
                            or not questGui.Visible
                            or not char.Parent          -- character respawned
                            or (not _charHumFinal or _charHumFinal.Health <= 0) -- player is dying
                ...
                if questGui.Visible then
                    ...
                    if target then
                        ...
                        repeat
                            ...
                            hrp.CFrame = target.HumanoidRootPart.CFrame * CFrame.new(0, 3, 0)
                            target.HumanoidRootPart.CanCollide = false
                            -- FIX: safe Humanoid access
                            local tHum2 = GetHumanoid(target)
                            if tHum2 then
                                tHum2.WalkSpeed = 0
                                tHum2.JumpHeight = 0
                            end
                        ...
                        local _tH2 = GetHumanoid(target)
                        local _cH2 = GetHumanoid(char)
                        until not _G.AutoFarmLevelNew
                            or (not _tH2 or _tH2.Health <= 0)
                            or not target.Parent
                            or not questGui.Visible
                            or (not _cH2 or _cH2.Health <= 0)  -- player is dying
        ...
        if _G.AutoNear then
            pcall(function()
                for _, v522 in pairs(game.Workspace.Enemies:GetChildren()) do
                    if v522:FindFirstChild("Humanoid") and v522:FindFirstChild("HumanoidRootPart") and (function() local _h=v522:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() and (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - v522.HumanoidRootPart.Position).Magnitude <= 5000 then
                        repeat
                            ...
                            pcall(function() local _h=v522:FindFirstChildOfClass("Humanoid") if _h then _h.JumpHeight=0 end end)
                            pcall(function() local _h=v522:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                            v522.HumanoidRootPart.CanCollide = false
                            FarmPos = v522.HumanoidRootPart.CFrame
                            MonFarm = v522.Name
                        until not _G.AutoNear or not v522.Parent or (function() local _h=v522:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)() or not game.Workspace.Enemies:FindFirstChild(v522.Name)
                        StartBring = false
                    end
                end
            end)
            ...
            pcall(function()
                ...
                if (CFrame.new(-5539.3115234375, 313.800537109375, -2972.372314453125).Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 500 then
                    for _, v546 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if _G.AutoRaidPirate and v546:FindFirstChild("HumanoidRootPart") and v546:FindFirstChild("Humanoid") and (function() local _h=v546:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() and (v546.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude < 2000 then
                            repeat
                                task.wait()
                                AutoHaki()
                                EquipWeapon(_G.SelectWeapon)
                            ...
                            until (function() local _h=v546:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)() or not v546.Parent or _G.AutoRaidPirate == false
                            NeedAttacking = false
                            StartMagnet = false
                        end
                    end
                ...
                if not game:GetService("Workspace").Enemies:FindFirstChild("Tyrant of the Skies") then
                    ...
                    if not v556 then
                        ...
                        for _, v561 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if (v561.Name == "Isle Outlaw" or v561.Name == "Island Boy" or v561.Name == "Isle Champion" or v561.Name == "Serpent Hunter" or v561.Name == "Skull Slayer") and v561:FindFirstChild("Humanoid") and v561:FindFirstChild("HumanoidRootPart") and (function() local _h=v561:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                repeat
                                    task.wait()
                                    AutoHaki()
                                    EquipWeapon(_G.SelectWeapon)
                                    v561.HumanoidRootPart.CanCollide = false
                                    pcall(function() local _h=v561:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                    StartBring = true
                    ...
                    for _, v563 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if v563.Name == "Tyrant of the Skies" and v563:FindFirstChild("Humanoid") and v563:FindFirstChild("HumanoidRootPart") and (function() local _h=v563:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                            repeat
                                task.wait()
                                AutoHaki()
                                EquipWeapon(_G.SelectWeapon)
                                v563.HumanoidRootPart.CanCollide = false
                                pcall(function() local _h=v563:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                v563.HumanoidRootPart.Size = Vector3.new(50, 50, 50)
                                topos(v563.HumanoidRootPart.CFrame * CFrame.new(0, 40, 0))
                                NeedAttacking = true
                            until not _G.FarmDaiBan or not v563.Parent or (function() local _h=v563:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                            task.wait(1)
                        end
                    end
                end
                ...
                pcall(function()
                    ...
                    if not game:GetService("Workspace").Enemies:FindFirstChild("Reborn Skeleton") and not game:GetService("Workspace").Enemies:FindFirstChild("Living Zombie") and not game:GetService("Workspace").Enemies:FindFirstChild("Demonic Soul") and not game:GetService("Workspace").Enemies:FindFirstChild("Posessed Mummy") then
                        ...
                        for _, v598 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if (v598.Name == "Reborn Skeleton" or v598.Name == "Living Zombie" or v598.Name == "Demonic Soul" or v598.Name == "Posessed Mummy") and v598:FindFirstChild("Humanoid") and v598:FindFirstChild("HumanoidRootPart") and (function() local _h=v598:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                repeat
                                    task.wait()
                                    AutoHaki()
                                    ...
                                    pcall(function() local _h=v598:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                ...
                                until not _G.FarmBone or not v598.Parent or (function() local _h=v598:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                            end
                        end
                    end
                end)
                ...
                if not game:GetService("Workspace").Enemies:FindFirstChild("Soul Reaper") then
                    ...
                    for _, v601 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if string.find(v601.Name, "Soul Reaper") then
                            ...
                            until (function() local _h=v601:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)() or _G.Hallow == false
                        end
                    end
                end
            end)
                ...
                if not game:GetService("Workspace").Enemies:FindFirstChild("Cake Prince") then
                    ...
                    if v611 then
                        for _, v615 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if (v615.Name == "Cookie Crafter" or v615.Name == "Cake Guard" or v615.Name == "Baking Staff" or v615.Name == "Head Baker") and v615:FindFirstChild("Humanoid") and v615:FindFirstChild("HumanoidRootPart") and (function() local _h=v615:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                repeat
                                    task.wait()
                                    AutoHaki()
                                    EquipWeapon(_G.SelectWeapon)
                                    v615.HumanoidRootPart.CanCollide = false
                                    pcall(function() local _h=v615:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                    StartBring = true
                                ...
                                until not _G.FarmCake or not v615.Parent or (function() local _h=v615:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)() or game:GetService("Workspace").Map.CakeLoaf.BigMirror.Other.Transparency == 0 or game:GetService("ReplicatedStorage"):FindFirstChild("Cake Prince [Lv. 2300] [Raid Boss]") or game:GetService("Workspace").Enemies:FindFirstChild("Cake Prince [Lv. 2300] [Raid Boss]")
                                DamageAura = false
                            end
                        end
                    else
                    ...
                    for _, v618 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if v618.Name == "Cake Prince" and v618:FindFirstChild("Humanoid") and v618:FindFirstChild("HumanoidRootPart") and (function() local _h=v618:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                            repeat
                                task.wait()
                                AutoHaki()
                                EquipWeapon(_G.SelectWeapon)
                                v618.HumanoidRootPart.CanCollide = false
                                pcall(function() local _h=v618:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                ...
                                NeedAttacking = true
                            until not _G.FarmCake or not v618.Parent or (function() local _h=v618:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                            task.wait(1)
                        end
                    end
                end
                    ...
                    if game.Players.LocalPlayer.Backpack:FindFirstChild("Sweet Chalice") or game.Players.LocalPlayer.Character:FindFirstChild("Sweet Chalice") then
                        ...
                        elseif game.Workspace.Enemies:FindFirstChild("Baking Staff") or game.Workspace.Enemies:FindFirstChild("Head Baker") or game.Workspace.Enemies:FindFirstChild("Cake Guard") or game.Workspace.Enemies:FindFirstChild("Cookie Crafter") then
                            for _, v621 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                if (v621.Name == "Baking Staff" or v621.Name == "Head Baker" or v621.Name == "Cake Guard" or v621.Name == "Cookie Crafter") and (function() local _h=v621:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                    repeat
                                        task.wait()
                                        AutoHaki()
                                        EquipWeapon(_G.SelectWeapon)
                                        ...
                                        pcall(function() local _h=v621:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                        ...
                                        game:GetService("VirtualUser"):CaptureController()
                                        game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 672))
                                    until _G.Fullykatakuri == false or game:GetService("ReplicatedStorage"):FindFirstChild("Cake Prince") or not v621.Parent or (function() local _h=v621:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                                end
                            end
                        else
                            CakeBring = false
                            ...
                            for _, v623 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                if v623.Name == "Dough King" then
                                    repeat
                                        ...
                                        game:GetService("VirtualUser"):CaptureController()
                                        game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 672))
                                    until _G.Fullykatakuri == false or not v623.Parent or (function() local _h=v623:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                                end
                            end
                        end
                    ...
                    elseif string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Diablo") or string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Deandre") or string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Urban") then
                        if not game:GetService("Workspace").Enemies:FindFirstChild("Diablo") and not game:GetService("Workspace").Enemies:FindFirstChild("Deandre") and not game:GetService("Workspace").Enemies:FindFirstChild("Urban") then
                            ...
                            for _, v626 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                if (v626.Name == "Diablo" or v626.Name == "Deandre" or v626.Name == "Urban") and v626:FindFirstChild("Humanoid") and v626:FindFirstChild("HumanoidRootPart") and (function() local _h=v626:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                    repeat
                                        task.wait()
                                        AutoHaki()
                                        EquipWeapon(_G.SelectWeapon)
                                        ...
                                        pcall(function() local _h=v626:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                    ...
                                    until _G.Fullykatakuri == false or (function() local _h=v626:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)() or not v626.Parent or game.Players.LocalPlayer.Backpack:FindFirstChild("God's Chalice") or game.Players.LocalPlayer.Character:FindFirstChild("God's Chalice")
                                end
                            end
                        end
                    end
...
if World1 then
    v658 = {
        ...
        "Bobby",
        "Yeti",
        "Fiend Yeti",          -- Fiend Yeti mutation boss on Turtle Island
        "Mob Leader",
        "Vice Admiral",
        "Warden",
        "Chief Warden",
            ...
            pcall(function()
                if not game:GetService("Workspace").Enemies:FindFirstChild(_G.SelectBoss) then
                    ...
                    for _, v662 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        -- FIX: Use GetHumanoid() for NPC structure
                        local bHum = GetHumanoid(v662)
                        if v662.Name == _G.SelectBoss and bHum and v662:FindFirstChild("HumanoidRootPart") and bHum.Health > 0 then
                            repeat
                                task.wait()
                                AutoHaki()
                                EquipWeapon(_G.SelectWeapon)
                                v662.HumanoidRootPart.CanCollide = false
                                local _bHum = GetHumanoid(v662)
                                if _bHum then _bHum.WalkSpeed = 0 end
                                v662.HumanoidRootPart.Size = Vector3.new(80, 80, 80)
                                topos(v662.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                            local _bHumF = GetHumanoid(v662)
                            until not _G.AutoBoss or not v662.Parent or (not _bHumF or _bHumF.Health <= 0)
                        end
                    end
                end
            end)
            ...
            pcall(function()
                ...
                for _, v669 in pairs(MaterialMon) do
                    for _, v671 in pairs(enemies) do
                        if string.find(v671.Name, v669, 1, true)
                            ...
                            and (function() local _h=v671:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                            matTarget = v671
                            break
                        end
                    end
                ...
                if matTarget then
                    ...
                    repeat
                        ...
                        pcall(function() local _h=matTarget:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                        pcall(function() local _h=matTarget:FindFirstChildOfClass("Humanoid") if _h then _h.JumpHeight=0 end end)
                    until not _G.AutoFarmMaterial
                        or not matTarget.Parent
                        or (function() local _h=matTarget:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                        or (function() local _c=lp2.Character local _h=_c and _c:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                    game:GetService("VirtualUser"):Button1Up(Vector2.new(1280, 672))
                    StartBring = false
                else
...
-- V6.0 FINAL: Auto Fishing — fully deferred and pcall-wrapped
-- CRASH 1: WaitForChild("FishReplicated") at top level — blocks entire script on Sea 1/2
--          where FishReplicated doesn't exist. Moved inside task.defer + pcall.
-- CRASH 2: require(Util.GetWaterHeightAtLocation) at top level — same issue.
-- CRASH 3: v682.GetAttribute(v682, "State") — wrong call syntax, should be v682:GetAttribute()
--          This threw "attempt to call a nil value" every frame the fishing loop ran.
--          All three fixed: everything is now lazy-loaded inside the toggle callback.
local l_LocalPlayer_10 = game.Players.LocalPlayer
local l_FishReplicated_0 = nil
local l_FishingRequest_0 = nil
local l_MaxLaunchDistance_0 = 50  -- safe default
local v679 = nil  -- GetWaterHeightAtLocation

-- Lazy-init fishing modules only when the feature is first enabled
local function initFishingModules()
    if l_FishReplicated_0 then return true end  -- already loaded
    local ok = pcall(function()
        local RS = game:GetService("ReplicatedStorage")
        local fr = RS:FindFirstChild("FishReplicated")
        if not fr then error("FishReplicated not found") end
        l_FishReplicated_0 = fr
        l_FishingRequest_0 = fr:WaitForChild("FishingRequest", 5)
        local cfg = fr:FindFirstChild("FishingClient")
        if cfg then
            local cfgMod = cfg:FindFirstChild("Config")
            if cfgMod then
                local ok2, cfgData = pcall(require, cfgMod)
                if ok2 and cfgData and cfgData.Rod then
                    l_MaxLaunchDistance_0 = cfgData.Rod.MaxLaunchDistance or 50
                end
            end
        end
        local utilMod = RS:FindFirstChild("Util") and RS.Util:FindFirstChild("GetWaterHeightAtLocation")
        if utilMod then
            local ok3, fn = pcall(require, utilMod)
            if ok3 and type(fn) == "function" then v679 = fn end
        end
    end)
    return ok and l_FishReplicated_0 ~= nil
end
...
task.spawn(function()
    while task.wait() do
        pcall(function()
            ...
            if not initFishingModules() then return end
            ...
            if _G.SelectedRod and (not v682 or v682.Name ~= _G.SelectedRod) then
                ...
                if l_FirstChild_2 then
                    local hum = l_Character_6 and l_Character_6:FindFirstChildOfClass("Humanoid")
                    if hum then hum:EquipTool(l_FirstChild_2) end
                    v682 = l_FirstChild_2
                end
            end
            if l_Character_6 and v681 and v682 and l_FishingRequest_0 then
                local waterY = v679 and v679(v681.Position) or v681.Position.Y
                local _, v686 = workspace:FindPartOnRayWithIgnoreList(
                    Ray.new(l_Character_6.Head.Position, v681.CFrame.LookVector * l_MaxLaunchDistance_0),
                    {l_Character_6, workspace:FindFirstChild("Characters"), workspace:FindFirstChild("Enemies")}
                )
                local v687 = v686 and Vector3.new(v686.X, math.max(v686.Y, waterY), v686.Z)
                -- FIXED: was v682.GetAttribute(v682, "State") — wrong syntax, crashes every frame
                local v688 = pcall(function() return v682:GetAttribute("State") end) and v682:GetAttribute("State")
                local v689 = pcall(function() return v682:GetAttribute("ServerState") end) and v682:GetAttribute("ServerState")
                if v688 ~= "ReeledIn" and v689 ~= "ReeledIn" or not v687 then
                    if v689 == "Biting" then
                        pcall(function() l_FishingRequest_0:InvokeServer("Catching", true) end)
                        task.wait(0.1)
                        pcall(function() l_FishingRequest_0:InvokeServer("Catch", 1) end)
                    end
                else
                    pcall(function() l_FishingRequest_0:InvokeServer("StartCasting") end)
                    task.wait()
                    pcall(function() l_FishingRequest_0:InvokeServer("CastLineAtLocation", v687, 100, true) end)
                end
            end
        end)
    end
...
v486:AddDropdown({
    ...
    Callback = function(v690)
        ...
        pcall(function()
            if initFishingModules() and l_FishingRequest_0 then
                    ...
                    if game.Players.LocalPlayer.Data.Level.Value >= 700 and World1 then
                        ...
                        elseif game:GetService("Workspace").Enemies:FindFirstChild("Ice Admiral") then
                            for _, v696 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                if v696.Name == "Ice Admiral" and (function() local _h=v696:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                    repeat
                                        task.wait()
                                        AutoHaki()
                                        EquipWeapon(_G.SelectWeapon)
                                    ...
                                    until (function() local _h=v696:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)() or not v696.Parent or not _G.AutoSecondSea
                                    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TravelDressrosa")
                                end
                            end
                        else
                ...
                pcall(function()
                    if game:GetService("Workspace").Enemies:FindFirstChild("Greybeard") then
                        for _, v700 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v700.Name == "Greybeard" and v700:FindFirstChild("Humanoid") and v700:FindFirstChild("HumanoidRootPart") and (function() local _h=v700:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                repeat
                                    task.wait()
                                    AutoHaki()
                                    EquipWeapon(_G.SelectWeapon)
                                    v700.HumanoidRootPart.CanCollide = false
                                    pcall(function() local _h=v700:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                    v700.HumanoidRootPart.Size = Vector3.new(50, 50, 50)
                                    topos(v700.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                    game:GetService("VirtualUser"):CaptureController()
                                    game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 672))
                                    sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", math.huge)
                                until not _G.Greybeard or not v700.Parent or (function() local _h=v700:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                            end
                        end
                    else
                    ...
                    if game:GetService("Workspace").Map.Jungle.Final.Part.Transparency ~= 0 then
                        if game:GetService("Workspace").Enemies:FindFirstChild("Saber Expert") or game:GetService("ReplicatedStorage"):FindFirstChild("Saber Expert") then
                            for _, v704 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                if v704:FindFirstChild("Humanoid") and v704:FindFirstChild("HumanoidRootPart") and (function() local _h=v704:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() and v704.Name == "Saber Expert" then
                                    repeat
                                        task.wait()
                                        EquipWeapon(_G.SelectWeapon)
                                        ...
                                        pcall(function() local _h=v704:FindFirstChildOfClass("Humanoid") if _h then _h.JumpHeight=0 end end)
                                        pcall(function() local _h=v704:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                        v704.HumanoidRootPart.CanCollide = false
                                        FarmPos = v704.HumanoidRootPart.CFrame
                                        MonFarm = v704.Name
                                        game:GetService("VirtualUser"):CaptureController()
                                        game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 672), workspace.CurrentCamera.CFrame)
                                    until (function() local _h=v704:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)() or not _G.AutoSaber
                                    if (function() local _h=v704:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)() then
                    ...
                    elseif game:GetService("Workspace").Enemies:FindFirstChild("Mob Leader") or game:GetService("ReplicatedStorage"):FindFirstChild("Mob Leader") then
                        ...
                        for _, v706 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v706.Name == "Mob Leader" then
                                if game:GetService("Workspace").Enemies:FindFirstChild("Mob Leader") and v706:FindFirstChild("Humanoid") and v706:FindFirstChild("HumanoidRootPart") and (function() local _h=v706:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                    repeat
                                        task.wait()
                                        AutoHaki()
                                        EquipWeapon(_G.SelectWeapon)
                                        v706.HumanoidRootPart.CanCollide = false
                                        pcall(function() local _h=v706:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                        v706.HumanoidRootPart.Size = Vector3.new(80, 80, 80)
                                        topos(v706.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                        game:GetService("VirtualUser"):CaptureController()
                                        game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 672))
                                        sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                                    until (function() local _h=v706:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)() or not _G.AutoSaber
                                end
                ...
                pcall(function()
                    if game:GetService("Workspace").Enemies:FindFirstChild("Thunder God") then
                        for _, v709 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v709.Name == "Thunder God" and v709:FindFirstChild("Humanoid") and v709:FindFirstChild("HumanoidRootPart") and (function() local _h=v709:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                repeat
                                    task.wait()
                                    AutoHaki()
                                    EquipWeapon(_G.SelectWeapon)
                                    ...
                                    StartBring = true
                                    pcall(function() local _h=v709:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                    v709.HumanoidRootPart.Size = Vector3.new(80, 80, 80)
                                    topos(v709.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                    sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                                until not _G.Autopole or not v709.Parent or (function() local _h=v709:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                            end
                        end
                    ...
                    pcall(function()
                        if not game:GetService("Workspace").Enemies:FindFirstChild("The Saw") then
                            ...
                            for _, v714 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                if v714.Name == "The Saw" and v714:FindFirstChild("Humanoid") and v714:FindFirstChild("HumanoidRootPart") and (function() local _h=v714:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                    repeat
                                        task.wait(_G.FastAttackDelay)
                                        AutoHaki()
                                        EquipWeapon(_G.SelectWeapon)
                                        v714.HumanoidRootPart.CanCollide = false
                                        pcall(function() local _h=v714:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                        v714.HumanoidRootPart.Size = Vector3.new(50, 50, 50)
                                        topos(v714.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                        AttackNoCD()
                                    until not _G.Autosaw or not v714.Parent or (function() local _h=v714:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                                end
                            end
                        end
                    end)
                    ...
                    pcall(function()
                        if game:GetService("Workspace").Enemies:FindFirstChild("Chief Warden") then
                            for _, v717 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                if v717.Name == "Chief Warden" and v717:FindFirstChild("Humanoid") and v717:FindFirstChild("HumanoidRootPart") and (function() local _h=v717:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                    repeat
                                        task.wait()
                                        AutoHaki()
                                        EquipWeapon(_G.SelectWeapon)
                                        ...
                                        StartBring = true
                                        pcall(function() local _h=v717:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                        v717.HumanoidRootPart.Size = Vector3.new(80, 80, 80)
                                        topos(v717.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                        sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                                    until not _G.ChiefWarden or not v717.Parent or (function() local _h=v717:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                                end
                            end
                    ...
                    pcall(function()
                        if game:GetService("Workspace").Enemies:FindFirstChild("Fishman Lord") then
                            for _, v720 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                if v720.Name == "Fishman Lord" and v720:FindFirstChild("Humanoid") and v720:FindFirstChild("HumanoidRootPart") and (function() local _h=v720:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                    repeat
                                        task.wait()
                                        AutoHaki()
                                        EquipWeapon(_G.SelectWeapon)
                                        ...
                                        StartBring = true
                                        pcall(function() local _h=v720:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                        v720.HumanoidRootPart.Size = Vector3.new(80, 80, 80)
                                        topos(v720.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                        sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                                    until not _G.Trident or not v720.Parent or (function() local _h=v720:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                                end
                            end
                                    ...
                                    if l_v724_0.Name == Ms then
                                        pcall(function()
                                            repeat
                                                ...
                                                game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 672))
                                                StartBring = true
                                            until not l_v724_0.Parent or (function() local _h=l_v724_0:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)() or _G.AutoBartilo == false or game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == false
                                            StartBring = false
                                        end)
                                    end
                                end
                        ...
                        for _, v727 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v727.Name == Ms then
                                ...
                                repeat
                                    ...
                                    game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 672))
                                    sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                                until not v727.Parent or (function() local _h=v727:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)() or _G.AutoBartilo == false
                            end
                        end
                    end
                end
                                ...
                                for _, v730 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                    if v730.Name == "rip_indra" then
                                        ...
                                        repeat
                                            ...
                                            StartBring = true
                                            pcall(function() local _h=v730:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TravelZou")
                                            sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                                        until _G.ThirdSea == false or (function() local _h=v730:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)() or not v730.Parent
                                    end
                                end
                            end
                        end
                ...
                if _G.AutoFactory then
                    if game:GetService("Workspace").Enemies:FindFirstChild("Core") then
                        for _, v734 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v734.Name == "Core" and (function() local _h=v734:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                repeat
                                    task.wait()
                                    AutoHaki()
                                    EquipWeapon(_G.SelectWeapon)
                                    ...
                                    game:GetService("VirtualUser"):CaptureController()
                                    game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 672))
                                until (function() local _h=v734:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)() or _G.AutoFactory == false
                            end
                        end
                    else
                        topos(CFrame.new(448.46756, 199.356781, -441.389252))
                ...
                pcall(function()
                    if not game:GetService("Workspace").Enemies:FindFirstChild("Darkbeard") then
                        ...
                        for _, v738 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v738.Name == "Darkbeard" and v738:FindFirstChild("Humanoid") and v738:FindFirstChild("HumanoidRootPart") and (function() local _h=v738:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                repeat
                                    task.wait()
                                    NeedAttacking = true
                                    AutoHaki()
                                    ...
                                    pcall(function() local _h=v738:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                    topos(v738.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                    sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                                until not _G.AutoDarkBoss or not v738.Parent or (function() local _h=v738:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                            end
                        end
                    end
                end)
                ...
                pcall(function()
                    if not game:GetService("Workspace").Enemies:FindFirstChild("Cursed Captain") then
                        ...
                        for _, v741 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v741.Name == "Cursed Captain" and v741:FindFirstChild("Humanoid") and v741:FindFirstChild("HumanoidRootPart") and (function() local _h=v741:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                repeat
                                    task.wait()
                                    NeedAttacking = true
                                    AutoHaki()
                                    ...
                                    pcall(function() local _h=v741:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                    topos(v741.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                    sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                                until not _G.CursedCaptain or not v741.Parent or (function() local _h=v741:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                            end
                        end
                    end
                end)
                ...
                pcall(function()
                    if game:GetService("Workspace").Enemies:FindFirstChild("Diamond") then
                        for _, v752 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v752.Name == "Diamond" and v752:FindFirstChild("Humanoid") and v752:FindFirstChild("HumanoidRootPart") and (function() local _h=v752:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                repeat
                                    task.wait()
                                    AutoHaki()
                                    EquipWeapon(_G.SelectWeapon)
                                    ...
                                    StartBring = true
                                    pcall(function() local _h=v752:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                    v752.HumanoidRootPart.Size = Vector3.new(80, 80, 80)
                                    topos(v752.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                    sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                                until not _G.Longsword or not v752.Parent or (function() local _h=v752:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                            end
                        end
                ...
                pcall(function()
                    ...
                    if not game:GetService("Workspace").Enemies:FindFirstChild("Orbitus") then
                        ...
                        for _, v755 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v755.Name == "Orbitus" and v755:FindFirstChild("Humanoid") and v755:FindFirstChild("HumanoidRootPart") and (function() local _h=v755:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                repeat
                                    task.wait()
                                    AutoHaki()
                                    EquipWeapon(_G.SelectWeapon)
                                    ...
                                    StartBring = true
                                    pcall(function() local _h=v755:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                    v755.HumanoidRootPart.Size = Vector3.new(80, 80, 80)
                                    topos(v755.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                    sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                                until not _G.GravityBlade or not v755.Parent or (function() local _h=v755:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                            end
                        end
                    end
                end)
                ...
                pcall(function()
                    if game:GetService("Workspace").Enemies:FindFirstChild("Smoke Admiral") then
                        for _, v758 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v758.Name == "Smoke Admiral" and v758:FindFirstChild("Humanoid") and v758:FindFirstChild("HumanoidRootPart") and (function() local _h=v758:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                repeat
                                    task.wait()
                                    AutoHaki()
                                    EquipWeapon(_G.SelectWeapon)
                                    ...
                                    StartBring = true
                                    pcall(function() local _h=v758:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                    v758.HumanoidRootPart.Size = Vector3.new(80, 80, 80)
                                    topos(v758.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                    sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                                until not _G.SwodsFlail or not v758.Parent or (function() local _h=v758:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                            end
                        end
                ...
                pcall(function()
                    if game:GetService("Workspace").Enemies:FindFirstChild("Tide Keeper") then
                        for _, v764 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v764.Name == "Tide Keeper" and v764:FindFirstChild("Humanoid") and v764:FindFirstChild("HumanoidRootPart") and (function() local _h=v764:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                repeat
                                    task.wait()
                                    AutoHaki()
                                    EquipWeapon(_G.SelectWeapon)
                                    ...
                                    StartBring = true
                                    pcall(function() local _h=v764:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                    v764.HumanoidRootPart.Size = Vector3.new(80, 80, 80)
                                    topos(v764.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                    sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                                until not _G.SwodsDRTrident or not v764.Parent or (function() local _h=v764:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                            end
                        end
                ...
                pcall(function()
                    ...
                    for _, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if v.Name == "Order" and v:FindFirstChild("Humanoid")
                           and v:FindFirstChild("HumanoidRootPart") and (function() local _h=v:FindFirstChildOfClass("Humanoid") return _h~=nil and _h.Health>0 end)() then
                            boss = v; break
                        end
                    end
                    if boss then
                        ...
                        pcall(function() local _h=boss:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                    ...
                    if _G.RipIndraKill then
                        if not game:GetService("Workspace").Enemies:FindFirstChild("rip_indra True Form") and not game:GetService("Workspace").Enemies:FindFirstChild("rip_indra") then
                            ...
                            for _, v771 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                ...
                                do
                                    local l_v771_0 = v771
                                    if l_Name_0 == v773 and (function() local _h=l_v771_0:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() and l_v771_0:IsA("Model") and l_v771_0:FindFirstChild("Humanoid") and l_v771_0:FindFirstChild("HumanoidRootPart") then
                                        repeat
                                            task.wait()
                                            pcall(function()
                                                AutoHaki()
                                        ...
                                        until _G.RipIndraKill == false or (function() local _h=l_v771_0:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                                    end
                                end
                            end
                        end
                                        ...
                                        if not l_FirstChild_3 or not l_FirstChild_3.Tablet or not l_FirstChild_3.Tablet:FindFirstChild("Segment1") then
                                            ...
                                            if l_Enemies_2 and l_Enemies_2:FindFirstChild("Living Zombie") then
                                                for _, v784 in pairs(l_Enemies_2:GetChildren()) do
                                                    if v784:FindFirstChild("HumanoidRootPart") and v784:FindFirstChild("Humanoid") and (function() local _h=v784:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() and v784.Name == "Living Zombie" then
                                                        AutoHaki()
                                                        EquipWeapon(getgenv().SelectWeapon)
                                                        v784.HumanoidRootPart.Size = Vector3.new(60, 60, 60)
                                                        v784.HumanoidRootPart.Transparency = 1
                                                        pcall(function() local _h=v784:FindFirstChildOfClass("Humanoid") if _h then _h.JumpHeight=0 end end)
                                                        pcall(function() local _h=v784:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                            ...
                            if string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Diablo") or string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Deandre") or string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Urban") then
                                if game:GetService("Workspace").Enemies:FindFirstChild("Diablo") or game:GetService("Workspace").Enemies:FindFirstChild("Deandre") or game:GetService("Workspace").Enemies:FindFirstChild("Urban") then
                                    for _, v795 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                        if (v795.Name == "Diablo" or v795.Name == "Deandre" or v795.Name == "Urban") and v795:FindFirstChild("Humanoid") and v795:FindFirstChild("HumanoidRootPart") and (function() local _h=v795:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                            repeat
                                                task.wait()
                                                AutoHaki()
                                                EquipWeapon(_G.SelectWeapon)
                                                ...
                                                pcall(function() local _h=v795:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                                topos(v795.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                                game:GetService("VirtualUser"):CaptureController()
                                                game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 672))
                                                sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                                            until _G.AutoElitehunter == false or (function() local _h=v795:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)() or not v795.Parent
                                        end
                                    end
                                else
                                    NeedAttacking = false
                ...
                pcall(function()
                    ...
                    if not l_Enemies_3:FindFirstChild("Cursed Skeleton Boss") then
                        ...
                        for _, v804 in pairs(l_Enemies_3:GetChildren()) do
                            if v804.Name == "Cursed Skeleton Boss" and v804:FindFirstChild("Humanoid") and v804:FindFirstChild("HumanoidRootPart") and (function() local _h=v804:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                ...
                                pcall(function() local _h=v804:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                ...
                                repeat
                                    task.wait()
                                until not getgenv().AutoGetCDK or not v804.Parent or (function() local _h=v804:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                            end
                        end
                    end
                end)
                    ...
                    pcall(function()
                        if game:GetService("Workspace").Enemies:FindFirstChild("Longma") then
                            for _, v811 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                if v811.Name == "Longma" and v811:FindFirstChild("Humanoid") and v811:FindFirstChild("HumanoidRootPart") and (function() local _h=v811:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                    repeat
                                        task.wait()
                                        AutoHaki()
                                        EquipWeapon(_G.SelectWeapon)
                                        ...
                                        StartBring = true
                                        pcall(function() local _h=v811:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                        v811.HumanoidRootPart.Size = Vector3.new(80, 80, 80)
                                        topos(v811.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                        sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                                    until not _G.AutoGetTushita or not v811.Parent or (function() local _h=v811:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                                end
                            end
                    ...
                    pcall(function()
                        if not game:GetService("Workspace").Enemies:FindFirstChild("Captain Elephant") then
                            ...
                            for _, v815 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                if v815.Name == "Captain Elephant" and v815:FindFirstChild("Humanoid") and v815:FindFirstChild("HumanoidRootPart") and (function() local _h=v815:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                    repeat
                                        task.wait()
                                        AutoHaki()
                                        EquipWeapon(_G.SelectWeapon)
                                        ...
                                        StartBring = true
                                        pcall(function() local _h=v815:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                        v815.HumanoidRootPart.Size = Vector3.new(80, 80, 80)
                                        topos(v815.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                        sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                                    until not _G.SwodTwinHooks or not v815.Parent or (function() local _h=v815:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                                end
                            end
                        end
                    end)
                    ...
                    pcall(function()
                        if game:GetService("Workspace").Enemies:FindFirstChild("Beautiful Pirate") then
                            for _, v818 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                if v818.Name == "Beautiful Pirate" and v818:FindFirstChild("Humanoid") and v818:FindFirstChild("HumanoidRootPart") and (function() local _h=v818:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                    repeat
                                        task.wait()
                                        AutoHaki()
                                        EquipWeapon(_G.SelectWeapon)
                                        ...
                                        StartBring = true
                                        pcall(function() local _h=v818:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                        v818.HumanoidRootPart.Size = Vector3.new(80, 80, 80)
                                        topos(v818.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                        sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                                    until not _G.SwodCanvander or not v818.Parent or (function() local _h=v818:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                                end
                            end
                    ...
                    pcall(function()
                        if not game:GetService("Workspace").Enemies:FindFirstChild("Cake Queen") then
                            ...
                            for _, v821 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                if v821.Name == "Cake Queen" and v821:FindFirstChild("Humanoid") and v821:FindFirstChild("HumanoidRootPart") and (function() local _h=v821:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                    repeat
                                        task.wait()
                                        AutoHaki()
                                        EquipWeapon(_G.SelectWeapon)
                                        ...
                                        StartBring = true
                                        pcall(function() local _h=v821:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                        v821.HumanoidRootPart.Size = Vector3.new(80, 80, 80)
                                        topos(v821.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                        sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                                    until not _G.SwodsBuddy or not v821.Parent or (function() local _h=v821:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                                end
                            end
                        end
                    end)
            ...
            pcall(function()
                if game:GetService("Workspace").Enemies:FindFirstChild("Lava Golem") then
                    for _, v924 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if v924.Name == "Lava Golem" and v924:FindFirstChild("Humanoid") and v924:FindFirstChild("HumanoidRootPart") and (function() local _h=v924:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                            repeat
                                task.wait()
                                AutoHaki()
                                EquipWeapon(_G.SelectWeapon)
                                v924.HumanoidRootPart.CanCollide = false
                                pcall(function() local _h=v924:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                                v924.HumanoidRootPart.Size = Vector3.new(50, 50, 50)
                                topos(v924.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", math.huge)
                            until not _G.KillGolem or not v924.Parent or (function() local _h=v924:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                        end
                    end
                else
                    UnEquipWeapon(_G.SelectWeapon)
                ...
                do
                    ...
                    if v928 then
                        for _, v931 in pairs(l_Children_0) do
                            do
                                local l_v931_0 = v931
                                if l_v931_0:FindFirstChild("Humanoid") and l_v931_0:FindFirstChild("HumanoidRootPart") and (function() local _h=l_v931_0:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() and (l_v931_0.HumanoidRootPart.Position - v928).Magnitude <= 1000 then
                                    pcall(function()
                                        repeat
                                            task.wait()
                                            sethiddenproperty(l_l_LocalPlayer_14_0, "SimulationRadius", math.huge)
                                            pcall(function() local _h=l_v931_0:FindFirstChildOfClass("Humanoid") if _h then _h.Health=0 end end)
                                            l_v931_0.HumanoidRootPart.CanCollide = false
                                        until not _G.Kill_Aura or not l_v931_0.Parent or (function() local _h=l_v931_0:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                                    end)
                                end
                            end
                        end
    ...
    while task.wait() do
        if _G.AutoLeviathan then
            pcall(function()
                ...
                if lev and lev:FindFirstChild("HumanoidRootPart") and lev:FindFirstChild("Humanoid") and (function() local _h=lev:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                    lev.HumanoidRootPart.CanCollide = false
                    pcall(function() local _h=lev:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                ...
                pcall(function()
                    ...
                    for _, enemy in pairs(enemies:GetChildren()) do
                        ...
                        if matched
                            ...
                            and (function() local _h=enemy:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                            repeat
                                task.wait()
                                AutoHaki()
                                EquipWeapon(_G.SelectWeapon)
                                enemy.HumanoidRootPart.CanCollide = false
                                pcall(function() local _h=enemy:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                            ...
                            until not flagGetter()
                                or not enemy.Parent
                                or (function() local _h=enemy:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                            break
                        end
                    end
                end)
                ...
                if game:GetService("Players").LocalPlayer.Data.Race.Value == "Human" then
                    for _, v1001 in pairs(game.Workspace.Enemies:GetDescendants()) do
                        do
                            ...
                            if l_v1001_0:FindFirstChild("Humanoid") and l_v1001_0:FindFirstChild("HumanoidRootPart") and (function() local _h=l_v1001_0:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                pcall(function()
                                    repeat
                                        task.wait(0.1)
                                        pcall(function() local _h=l_v1001_0:FindFirstChildOfClass("Humanoid") if _h then _h.Health=0 end end)
                                        l_v1001_0.HumanoidRootPart.CanCollide = false
                                        sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", math.huge)
                                    until not _G.AutoQuestRace or not l_v1001_0.Parent or (function() local _h=l_v1001_0:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                                end)
                            end
                        end
                    end
                    ...
                    elseif game:GetService("Players").LocalPlayer.Data.Race.Value == "Ghoul" then
                        for _, v1006 in pairs(game.Workspace.Enemies:GetDescendants()) do
                            do
                                ...
                                if l_v1006_0:FindFirstChild("Humanoid") and l_v1006_0:FindFirstChild("HumanoidRootPart") and (function() local _h=l_v1006_0:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                                    pcall(function()
                                        repeat
                                            task.wait(0.1)
                                            pcall(function() local _h=l_v1006_0:FindFirstChildOfClass("Humanoid") if _h then _h.Health=0 end end)
                                            l_v1006_0.HumanoidRootPart.CanCollide = false
                                            sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", math.huge)
                                        until not _G.AutoQuestRace or not l_v1006_0.Parent or (function() local _h=l_v1006_0:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
                                    end)
                                end
                            end
                        end
        ...
        if _G.AutoKillV4 then
            pcall(function()
                for _, v1022 in pairs(game.Workspace.Characters:GetChildren()) do
                    if v1022.Name ~= game.Players.LocalPlayer.Name and v1022:FindFirstChild("Humanoid") and v1022:FindFirstChild("HumanoidRootPart") and (function() local _h=v1022:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() and v1022.Parent and (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - v1022.HumanoidRootPart.Position).Magnitude <= 230 then
                        repeat
                            task.wait()
                            AutoHaki()
                            EquipWeapon(_G.SelectWeapon)
                            ...
                            pcall(function() local _h=v1022:FindFirstChildOfClass("Humanoid") if _h then _h.WalkSpeed=0 end end)
                            sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", math.huge)
                        until not _G.AutoKillV4 or (function() local _h=v1022:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)() or not v1022.Parent or not v1022:FindFirstChild("HumanoidRootPart") or not v1022:FindFirstChild("Humanoid")
                    end
                end
            end)
        end
...
local function raidKillNearby()
    ...
    for _, mob in pairs(workspace.Enemies:GetChildren()) do
        if mob:FindFirstChild("HumanoidRootPart") and mob:FindFirstChild("Humanoid")
           and (function() local _h=mob:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)()
           and (mob.HumanoidRootPart.Position - hrp.Position).Magnitude <= 1200 then
            repeat
                task.wait(0.1)
                if mob:FindFirstChild("Humanoid") and (function() local _h=mob:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() then
                    EquipWeapon(_G.SelectWeapon)
                    topos(mob.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                end
            until not mob:FindFirstChild("Humanoid") or (function() local _h=mob:FindFirstChildOfClass("Humanoid") return not _h or _h.Health <= 0 end)()
        end
    end
end
task.spawn(function()
        ...
        pcall(function()
            ...
            for _, v in pairs(workspace.Enemies:GetChildren()) do
                if v.Name == "Order"
                   ...
                   and v:FindFirstChild("HumanoidRootPart")
                   and (function() local _h=v:FindFirstChildOfClass("Humanoid") return _h~=nil and _h.Health>0 end)() then
                    boss = v
                    break
                end
            end
    ...
    while task.wait() do
        pcall(function()
            ...
            for _, v1167 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                if _G.BringMonster and (StartBring and v1167.Name == MonFarm or string.find(v1167.Name, Mon or '', 1, true) and v1167:FindFirstChild("Humanoid") and v1167:FindFirstChild("HumanoidRootPart") and (function() local _h=v1167:FindFirstChildOfClass("Humanoid") return _h ~= nil and _h.Health > 0 end)() and (v1167.HumanoidRootPart.Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 320) then
                    if v1167.Name == "Factory Staff" then
                        if (v1167.HumanoidRootPart.Position - PosMon.Position).Magnitude <= 250 then
                            ...
                            if v1167.Humanoid:FindFirstChild("Animator") then
                                pcall(function() local _h=v1167:FindFirstChildOfClass("Humanoid") if _h and _h:FindFirstChildOfClass("Animator") then _h:FindFirstChildOfClass("Animator"):Destroy() end end)
                            end
                            sethiddenproperty(game:GetService("Players").LocalPlayer, "SimulationRadius", math.huge)
                        end
...
-- V3.0 REFACTORED: Infinite Soru Cooldown
-- OLD: getgc + getfenv + debug.getupvalues + setupvalue (all banned, all crash in BF 2026)
-- NEW: VirtualInputManager key-hold approach — no debug, no getgc, stable across updates.
-- Limitation: does not force-reset internal cooldown variable, but continuously re-fires
-- the Soru input so the player experiences near-zero cooldown in practice.
v496:AddToggle({Name = "Infinite Soru Cooldown", Value = false, Callback = function(v1173)
    ...
    if v1173 then
        pcall(function()
            game.StarterGui:SetCore("SendNotification", {
                Title = "⚡ Soru Active",
                Text = "VirtualInput mode (stable, no debug)",
                Duration = 3
            })
        end)
    end
...
spawn(function()
    while task.wait(0.05) do
        pcall(function()
            ...
            -- Re-fire Soru input so cooldown is immediately consumed again
            game:GetService("VirtualInputManager"):SendKeyEvent(true,  "Y", false, game)
            task.wait(0.016)
            game:GetService("VirtualInputManager"):SendKeyEvent(false, "Y", false, game)
        end)
...
-- V3.0 REFACTORED: Dodge No Cooldown
-- OLD: getgc() + getfenv() + getupvalues() + setupvalue() — all banned, all crash
-- NEW: Fire the server Dodge remote directly from a tight loop instead of manipulating
--      the LocalScript's internal "0.4" cooldown upvalue via the GC scanner.
--      This achieves equivalent server-side effect with zero debug API usage.
PosY = 30
...
spawn(function()
    while task.wait(0.08) do
        pcall(function()
            if not DodgewithoutCool then return end
            local lp = game:GetService("Players").LocalPlayer
            local char = lp and lp.Character
            local hum = char and char:FindFirstChildOfClass("Humanoid")
            if not hum or hum.Health <= 0 then return end
            local hrp = char:FindFirstChild("HumanoidRootPart")
            if not hrp then return end
            -- Fire Dodge server remote (CommF_ "Dodge" action)
            local rs = game:GetService("ReplicatedStorage")
            local comm = rs:FindFirstChild("Remotes") and rs.Remotes:FindFirstChild("CommF_")
            if comm then
                comm:FireServer("Dodge", hrp.CFrame.LookVector)
            end
        end)
    end
end)
-- V3.0 REFACTORED: Infinite Geppo
-- OLD: getgc() + getfenv() + getupvalues() + setupvalue() — all banned
-- NEW: VirtualInputManager space-hold loop. Geppo is triggered by Space in BF.
--      Hammering the input keeps the jump available without touching the closure.
...
spawn(function()
    while task.wait(0.05) do
        pcall(function()
            if not InfiniteGeppo then return end
            local lp = game:GetService("Players").LocalPlayer
            local char = lp and lp.Character
            local hum = char and char:FindFirstChildOfClass("Humanoid")
            if not hum or hum.Health <= 0 then return end
