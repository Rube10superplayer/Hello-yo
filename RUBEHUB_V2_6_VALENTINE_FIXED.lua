...
-- VALENTINE'S 2026 UPDATE FIX - V2.6
-- File Size: ~600KB | All Features + Valentine's 2026 Event
--
-- VALENTINE'S 2026 FIXES APPLIED (v2.6):
-- ✅ Fixed: "Humanoid is not a valid member of Model" error
--    → Added GetHumanoid() helper that searches sub-models (NPC structure changed in Feb 2026 update)
-- ✅ Fixed: "attempt to call a nil value" at LocalScript Line 1
--    → hookfunction on Effect.Container.Death was crashing because the module path
--      changed in the Valentine's update. Now uses safe multi-path lookup.
-- ✅ Added: Valentine's 2026 event section
--    → Auto Cupid Daily Quest (all 3 seas)
--    → Auto Fiend Yeti boss farm (Turtle Island, Sea 3)
--    → Auto Delivery Quest (flamingo missions)
--    → Teleport buttons to event NPCs
-- ✅ Added: Fiend Yeti to World1 boss dropdown
-- ✅ Fixed: Boss Farm loop uses GetHumanoid() instead of direct .Humanoid access
-- ✅ Fixed: AutoFarm & AutoFarmLevelNew loops use GetHumanoid()
-- ✅ Fixed: BTP() function uses FindFirstChildOfClass("Humanoid")
--
...
-- ═══════════════════════════════════════════════════════════════
-- SAFE HOOKFUNCTION WRAPPER (Valentine's 2026 Update Fix)
-- The Effect.Container path changed in the Valentine's update.
-- We now try multiple known paths and only hook if the result
-- is actually a callable function (not a table/nil).
-- ═══════════════════════════════════════════════════════════════
pcall(function()
    if not hookfunction then return end

    -- Helper: safely require a module and return its main function
    local function safeRequireFunc(path)
        ...
        elseif type(result) == "table" then
            -- Some modules export their main function as result[1] or result.run
            if type(result[1]) == "function" then return result[1]
            elseif type(result.run) == "function" then return result.run
            elseif type(result.execute) == "function" then return result.execute
            end
        end
        return nil
    end
    ...
    -- Try known Death module paths (Valentine's 2026 may have moved it)
    local deathPaths = {
        RS:FindFirstChild("Effect") and RS.Effect:FindFirstChild("Container") and RS.Effect.Container:FindFirstChild("Death"),
        RS:FindFirstChild("Effects") and RS.Effects:FindFirstChild("Death"),
        RS:FindFirstChild("Effect") and RS.Effect:FindFirstChild("Death"),
    }
    for _, path in pairs(deathPaths) do
        if path then
            local fn = safeRequireFunc(path)
            if fn then
                pcall(hookfunction, fn, function() end)
                print("✅ Death hookfunction applied via: " .. path:GetFullName())
                break
            end
        end
    end

    -- Try known Respawn module paths
    local respawnPaths = {
        RS:FindFirstChild("Effect") and RS.Effect:FindFirstChild("Container") and RS.Effect.Container:FindFirstChild("Respawn"),
        RS:FindFirstChild("Effects") and RS.Effects:FindFirstChild("Respawn"),
        RS:FindFirstChild("Effect") and RS.Effect:FindFirstChild("Respawn"),
    }
    for _, path in pairs(respawnPaths) do
        if path then
            local fn = safeRequireFunc(path)
            if fn then
                pcall(hookfunction, fn, function() end)
                print("✅ Respawn hookfunction applied via: " .. path:GetFullName())
                break
            end
        end
    end
