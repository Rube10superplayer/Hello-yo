...
-- V3.0 REFACTORED: No getupvalues. CombatFramework is accessed directly as a table.
-- require() in BF returns a table; .activeController is a public field on it.
-- If the structure changes BF-side, the pcall silently fails and FastAttack is skipped.
local CombatFramework, CombatFrameworkR, realbhit
pcall(function()
    local ps = game:GetService("Players").LocalPlayer:WaitForChild("PlayerScripts", 5)
    if not ps then return end
    local cfScript = ps:FindFirstChild("CombatFramework")
    if not cfScript then return end
    CombatFramework = require(cfScript)
    -- CombatFramework module returns itself as a table with .activeController
    -- We do NOT use getupvalues — access the table directly.
    if type(CombatFramework) == "table" then
        CombatFrameworkR = CombatFramework
    end
    -- RigLib for blade hit detection
    local rigLib = game:GetService("ReplicatedStorage"):FindFirstChild("CombatFramework")
    if rigLib then
        local rl = rigLib:FindFirstChild("RigLib")
        if rl then pcall(function() realbhit = require(rl) end) end
    end
end)
