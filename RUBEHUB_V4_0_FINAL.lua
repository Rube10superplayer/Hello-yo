-- RubeHub V3.0 REFACTORED | Credits: RubeTheCoder
-- ═══════════════════════════════════════════════════════════════
-- COMPLETE STABILITY REFACTOR — WHAT WAS DANGEROUS AND WHY
-- ═══════════════════════════════════════════════════════════════
--
-- ❌ REMOVED: getgc()
--    WHERE: Infinite Soru (×1), Dodge No Cooldown (×1), Infinite Geppo (×1)
--    WHY IT CRASHED: getgc() returns every Lua closure AND C-closure alive in
--    the VM. Calling getfenv() on a C-closure throws "attempt to get environment
--    of non-Lua function". Even inside pcall, some executors propagate this as
--    an unhandled coroutine error reported at "LocalScript Line 1". Additionally,
--    scanning thousands of closures every frame (the Dodge loop ran every tick)
--    is an O(n) GC walk that spikes memory and framerate.
--    FIX: Replaced with VirtualInputManager key-fire loops (Soru=Y, Geppo=Space)
--    and a CommF_ remote-fire loop (Dodge). Same server-side result, zero debug.
--
-- ❌ REMOVED: debug library (debug.getupvalues, debug.getupvalue, debug.setupvalue,
--             debug.getfenv)
--    WHERE: Exploit compat layer (lines ~60, ~139) and the three cooldown features.
--    WHY IT CRASHED: The debug library is not guaranteed to exist in all executors.
--    `debug and debug.getfenv` evaluates fine if debug == nil, but once assigned to
--    a local and called on a C-closure it throws. Assigning `debug.*` as a fallback
--    for setupvalue then CALLING setupvalue on a game-internal closure crashes the
--    thread. BF 2026 closures are identity-protected on the server — writing an
--    upvalue triggers a protection fault in newer executor sandboxes.
--    FIX: All debug.* fallbacks replaced with safe no-op lambdas.
--
-- ❌ REMOVED: getupvalues(CombatFramework)[2] pattern
--    WHERE: Fast Attack CombatFramework init block.
--    WHY IT CRASHED: getupvalues(module)[2] assumes the module's second upvalue is
--    the internal state table. After BF 2026's CombatFramework rewrite the second
--    upvalue is no longer the activeController table — it returns a function or nil,
--    and indexing nil[2] throws. Even when it works, any BF update silently breaks
--    it by shifting upvalue indices.
--    FIX: require() in BF returns the module table directly. CombatFrameworkR is now
--    set to CombatFramework (the table itself) and .activeController is accessed as
--    a normal field. If the structure ever changes, pcall catches it safely.
--
-- ❌ REMOVED: hookfunction on Effect.Container closures
--    (documented in V3.0 original header — kept here for completeness)
--    WHY IT CRASHED: BF 2026 uses protected closures in its Effect modules.
--    hookfunction on a protected closure throws immediately.
--    FIX: pcall(require, child) pre-warm only, no hooking.
--
-- ❌ REMOVED: LocalScript :Destroy() inside EffectsLocalThread watcher
--    WHY IT IS DANGEROUS: Destroying the parent LocalScript from its own Disabled
--    watcher creates a re-entrant destroy that can corrupt the thread pool.
--    FIX: .Disabled = true is set FIRST via pcall, then :Destroy() in a separate
--    pcall so either can fail independently without cascading.
--
-- ✅ KEPT SAFE: CombatFramework.activeController — direct field access only
-- ✅ KEPT SAFE: GetHumanoid() helper — FindFirstChildOfClass everywhere
-- ✅ KEPT SAFE: Effect.Container — pcall(require) only, no hookfunction
-- ✅ KEPT SAFE: Fast Attack — pcall-wrapped, nil-checked, rate-limited (30/s)
-- ✅ KEPT SAFE: All exploit function fallbacks — no-op lambdas, no debug.*
-- ✅ ADDED: Soru/Dodge/Geppo VirtualInputManager replacements
-- ═══════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════
-- V3.0 SAFE: Kill EffectsLocalThread (METHOD 1 only — safe, no getgc hooks)
-- ═══════════════════════════════════════════════════════════════
pcall(function()
    ...
    local function killEffectsScript(child)
        if child.Name == "EffectsLocalThread"
        or child.Name == "EffectLocalThread"
        or child.Name == "EffectsLocal" then
            pcall(function() child.Disabled = true end)
            pcall(function() child:Destroy() end)
        end
    end
    for _, child in pairs(ps:GetChildren()) do killEffectsScript(child) end
    ps.ChildAdded:Connect(killEffectsScript)
    print("✅ V3.0: EffectsLocalThread watcher active")
end)
